"""Vectorized index MaxSim must match dict-based reference."""
...
def test_vectorized_matches_dict_topk():
    index = build_chunk_index(_tiny_corpus(), n_latents=8, block_size=4)
    query = _tiny_corpus()[0]
    ref = exact_maxsim_via_index(
        query, index, top_docs=2, use_vectorized=False
    )
    fast = exact_maxsim_via_index(
        query, index, top_docs=2, use_vectorized=True
    )
    np.testing.assert_array_equal(np.sort(ref[0]), np.sort(fast[0]))
    np.testing.assert_allclose(
        np.sort(ref[1])[::-1], np.sort(fast[1])[::-1], rtol=0, atol=1e-5
    )
